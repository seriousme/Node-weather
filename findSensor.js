...
function processSensorData(err,body){
	if (!err){
		if (body.rows.length == 2) {
			...
			var found=false;
			console.log("Sensor",sensor0id,": aantal metingen",sensor0.count,"minumum", sensor0.min," maximum", sensor0.max);
			console.log("Sensor",sensor1id,": aantal metingen",sensor1.count,"minumum", sensor1.min," maximum", sensor1.max);
			...
			if (found){
				...
				weatherdb.insert(configRec,function(err, body) {
					  if (err)
						console.log("err: config update failed", err);
					});
			}
			else {
				console.log("Niet genoeg informatie om te bepalen welke sensor waar zit, probeer het later nogmaals");
			}
...
weatherdb.get('config/sensorIDs', function(err, body) {
	if (!err){
		...
		weatherdb.view('data', 'unknownSensors', { "reduce" : true, "group_level":2 },processSensorData );
	}
	else {
		console.log("err:", err.reason);
	}
